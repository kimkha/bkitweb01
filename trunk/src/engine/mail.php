...
class BKITMail {
	...
	function send(){                        									//Sending mail
		...
		$header='From:'.$this->from_name.'<'.$this->from_address.'>'."\r\n".    //
	...
	function load_template_from_file($template_file){       //Loading template  
		$filename="../mail_template/".$template_file;
		$f=fopen($filename,"r");                       		//Opening email template file in read-only mode
		$this->template = fread($f,filesize($filename));	//Getting the content of template and putting it into $template variable
	...
	function save_template_from_file($template_file){       //Saving current template
		...
		fwrite($f,$this->template);  						//Putting the content of template into template file
		fclose($f);                                         //Closing template file  
	}
